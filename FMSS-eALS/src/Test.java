...
public class Test {
    public static void test() {
        ...
        int UserNum_TestData = Data.TestData.keySet().size();

        for (int u = 1; u <= Data.n; u++) {
            // --- check whether the user $u$ is in the test set
            if (!Data.TestData.containsKey(u))
                continue;

            // ---
            HashSet<Integer> ItemSet_u_PurchaseData = new HashSet<Integer>();
            if (Data.I_u.containsKey(u)) {
                ItemSet_u_PurchaseData = Data.I_u.get(u);
            }
            ...
            for (int i = 1; i <= Data.m; i++) {
                ...
                if (!Data.ItemSetWhole.contains(i) || ItemSet_u_PurchaseData.contains(i))
                    continue;
